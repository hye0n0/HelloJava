...
public class BooksApp {
	...
	public static void BookM(String id, int userNum) {
		BooksDAO dao = new BooksDAO();
		Scanner scn = new Scanner(System.in);
		if (userNum == 1) { // 매니저 전용메뉴
			while (true) {
				...
				System.out.println(
						"==============================================================================================================");
				System.out.println("메뉴)) 1.책등록 2.책수정 3.책삭제 4.전체목록 5.상세조회 6.내도서후기보기 7.북마크 8.대여중인도서 9.회원관리 10.QNA 0.로그아웃");
				int menu = readInt("선택>>  ");
				if (menu == 1) { // 책등록
					while (true) {
						System.out.println();
					...
					while (true) {
						...
						if (search == -1) {
							System.out
									.println("설정)) 현재 페이지 : " + page + " | 글목록: " + pageSet + "개씩 | 순서: " + orderWord);
						} else {
						...
						for (int i = ((page - 1) * pageSet); i < (page * pageSet); i++) {
							if (i == books.size()) {
								...
								System.out.printf("책번호: %3s | 책이름: %10s | 글쓴이: %5s | 출판사: %7s \n", books.get(i).getBookId(), books.get(i).getBookName(), books.get(i).getBookWriter(), books.get(i).getBookPublisher());
							}
						}
						System.out.println();
						System.out.println(
					...
					while (true) {
						...
						for (Bookmark bmk : bmks) {
							System.out.printf("북마크번호: %3s | 책번호: %3s | 책이름: %10s | 글쓴이: %5s | 출판사: %7s \n",bmk.getMarkId(),bmk.getBookId(),bmk.getBookName(),bmk.getBookWriter(),bmk.getBookPublisher());
						}
						System.out.println(
								"==============================================================================================================");
					...
					while (true) {
						...
						for (Book rbook : rentBooks) {
							System.out.printf("책번호: %3s | 책이름: %10s | 글쓴이: %5s | 출판사: %7s \n", rbook.getBookId(), rbook.getBookName(), rbook.getBookWriter(), rbook.getBookPublisher());
						}
						System.out.println(
								"==============================================================================================================");
